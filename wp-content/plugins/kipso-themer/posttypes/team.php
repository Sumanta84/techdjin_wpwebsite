...
if(!function_exists('gavias_post_type_team')){
  ...
  function gaviasthemer_get_team_via_userId($userId){
      ...
      if ( ($team_socials) && count( $team_socials ) > 0 ) {
          foreach( $team_socials as $social ) {
              if ( isset( $social['icon'] ) || isset( $social['link'] ) ) {
                  printf( '<p><input size="24" type="text" placeholder="Icon, ex: fab fa-facebook" name="team_socials[%1$s][icon]" value="%2$s" /><input size="100" type="text" placeholder="Link" name="team_socials[%1$s][link]" value="%3$s" /><a class="button remove">%4$s</a></p>', $c, $social['icon'], $social['link'], __( 'Remove' ) );
                  $c = $c +1;
              }
          }
      }
  ...
  <script>
      ...
      $(document).ready(function() {
          ...
          $(".add-social-item").click(function() {
              count = count + 1;
              $('#team-social-list').append('<p> <input size="24" type="text" placeholder="Icon, ex: fab fa-facebook" name="team_socials['+count+'][icon]" value="" /><input size="100" type="text" placeholder="Link" name="team_socials['+count+'][link]" value="" /> <a class="remove button">Remove</a></p>' );
              return false;
          });
  ...
  // -- Dynamic Skills User Metabox -- 
  function gavias_additional_profile_skills( $user ) {
    ?>
      <h3>Education</h3>
    ...
    <div id="meta_inner">
      <?php
        $team_educations = get_the_author_meta( 'team_educations', $user->ID );
        $c = 0;
        if ( ($team_educations) && count( $team_educations ) > 0 ) {
          foreach( $team_educations as $education ) {
            if ( isset( $education['title'] ) || isset( $education['desc'] ) ) {
              printf( '<p><input size="80" type="text" placeholder="Title" name="team_educations[%1$s][title]" value="%2$s" /><input size="50" type="text" placeholder="Complete Training Web Developer" name="team_educations[%1$s][desc]" value="%3$s" /><a class="button remove">%4$s</a></p>', $c, $education['title'], $education['desc'], __( 'Remove' ) );
              $c = $c +1;
            }
          }
        }
    ...
    <script>
      ...
      $(document).ready(function() {
          ...
          $(".add-education-item").click(function() {
              count = count + 1;
              $('#team-education-list').append('<p><input size="80" type="text" placeholder="Title" name="team_educations['+count+'][title]" value="" /> <input size="50" type="text" placeholder="Complete Training Web Developer" name="team_educations['+count+'][desc]" value="" /><a class="remove button">Remove</a></p>' );
              return false;
          });
          $(".remove").on('click', function() {
              $(this).parent().remove();
    ...
    <div id="meta_inner">
      <?php
        $team_skills = get_the_author_meta( 'team_skills', $user->ID );
        $c = 0;
  ...
  add_action( 'show_user_profile', 'gavias_additional_profile_skills' );
  add_action( 'edit_user_profile', 'gavias_additional_profile_skills' );

  function gavias_save_profile_skills( $user_id ) {
    if ( ! current_user_can( 'edit_user', $user_id ) ) {
     return false;
    }

    if ( !empty( $_POST['team_educations'] ) ) {
     update_usermeta( $user_id, 'team_educations', $_POST['team_educations'] );
    }

    if ( !empty( $_POST['team_skills'] ) ) {
     update_usermeta( $user_id, 'team_skills', $_POST['team_skills'] );
    }

    
  }

  add_action( 'personal_options_update', 'gavias_save_profile_skills' );
  add_action( 'edit_user_profile_update', 'gavias_save_profile_skills' );
