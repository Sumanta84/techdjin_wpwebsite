...
if ( ! function_exists( 'kipso_custom_styles_save' ) ) :
function kipso_custom_styles_save() {

	$main_font = false;
	$main_font_enabled = ( kipso_get_option('main_font_source', 0) == 0 ) ? false : true;
	if ( $main_font_enabled ) {
		$font_main = kipso_get_option('main_font', '');
	...
	$secondary_font_enabled = ( kipso_get_option('secondary_font_source', 0) == 0 ) ? false : true;
	if ( $secondary_font_enabled ) {
		$font_second = kipso_get_option('secondary_font', '');
...
/* ----- Main Color ----- */
<?php if($style = kipso_get_option('main_color', '')){ ?>
	body{
...
/* ----- Background body ----- */
<?php 
	$main_background = kipso_get_option('main_background_image', '');
...
body{
	...
	<?php if ( kipso_get_option('main_background_image_type', '') == 'fixed' ) : ?>
	...
	<?php endif; endif; ?>
	background-color:<?php echo esc_attr( kipso_get_option('main_background_color', '') ); ?>;
}
<?php } ?>

/* ----- Main content ----- */
<?php if(kipso_get_option('content_background_color', '')){ ?>
div.page {
	background: <?php echo esc_attr( kipso_get_option('content_background_color', '') ); ?>!important;
}
<?php } ?>

<?php if(kipso_get_option('content_font_color', '')){ ?>
div.page {
	color: <?php echo esc_attr( kipso_get_option('content_font_color', '') ); ?>;
}
<?php } ?>

<?php if(kipso_get_option('content_font_color_link', '')){ ?>
div.page a{
	color: <?php echo esc_attr( kipso_get_option('content_font_color_link', '') ); ?>;
}
<?php } ?>

<?php if(kipso_get_option('content_font_color_link_hover', '')){ ?>
div.page a:hover, div.page a:active, div.page a:focus {
	background: <?php echo esc_attr( kipso_get_option('content_font_color_link_hover', '') ); ?>!important;
}
<?php } ?>

/* ----- Footer content ----- */
<?php if(kipso_get_option('footer_background_color', '')){ ?>
#wp-footer {
	background: <?php echo esc_attr( kipso_get_option('footer_background_color', '') ); ?>!important;
}
<?php } ?>

<?php if(kipso_get_option('footer_font_color', '')){ ?>
#wp-footer {
	color: <?php echo esc_attr( kipso_get_option('footer_font_color', '') ); ?>;
}
<?php } ?>

<?php if(kipso_get_option('footer_font_color_link', '')){ ?>
#wp-footer a{
	color: <?php echo esc_attr( kipso_get_option('footer_font_color_link', '') ); ?>;
}
<?php } ?>

<?php if(kipso_get_option('footer_font_color_link_hover', '')){ ?>
#wp-footer a:hover, #wp-footer a:active, #wp-footer a:focus {
	background: <?php echo esc_attr( kipso_get_option('footer_font_color_link_hover', '') ); ?>!important;
}
<?php } ?>
...
<?php
	...
	update_option( 'kipso_theme_custom_styles', $styles, true );
}
endif;

add_action( 'redux/options/kipso_theme_options/saved', 'kipso_custom_styles_save' );


/* Make sure custom theme styles are saved */
function kipso_custom_styles_install() {
	if ( ! get_option( 'kipso_theme_custom_styles' ) && get_option( 'kipso_theme_options' ) ) {
		kipso_custom_styles_save();
	}
}

add_action( 'redux/options/kipso_theme_options/register', 'kipso_custom_styles_install' );
