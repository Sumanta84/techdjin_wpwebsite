...
$message_confirm_complete_item = sprintf( '%s "%s"?', __( 'Do you want to complete lesson', 'kipso' ), $item->get_title() );
...
if ( $completed ) :
	...
	<div>
		...
		echo sprintf(
			'%s %s',
			esc_html__( 'You have completed this lesson at ', 'kipso' ),
			$user_item_data->get_end_time()->format( 'i18n_has_time' )
		)
		?>
	</div>
	<button class="lp-button completed" disabled>
		<i class="fa fa-check"></i><?php esc_html_e( 'Completed', 'kipso' ); ?>
	</button>
<?php else : ?>
	...
	<form method="post" name="learn-press-form-complete-lesson"
		  ...
		  data-title="<?php echo esc_attr(__('Complete lesson', 'kipso'))?>"
		...
		<button class="lp-button button button-complete-item button-complete-lesson lp-btn-complete-item">
			<?php echo esc_html__( 'Complete', 'kipso' ); ?>
		</button>
