...
<li <?php $section->main_class(); ?>
	...
	<div class="section-header">
		<div class="section-left">
			<h5 class="section--title">
				<?php
					...
					echo ! $title ? _x( 'Untitled', 'template title empty', 'kipso' ) : $title;
				?>
		...
		<?php if ( $user->has_enrolled_course( $section->get_course_id() ) ) : ?>
			...
			<div class="section-meta">
				<div class="learn-press-progress" title="<?php echo esc_attr( sprintf( __( 'Section progress %s%%', 'kipso' ), round( $percent, 2 ) ) ); ?>">
	...
	<?php if ( ! $items ) : ?>
		<?php learn_press_display_message( __( 'No items in this section', 'kipso' ) ); ?>
	<?php else : ?>
		...
		<ul class="section-content">
			...
			foreach ( $items as $item ) :
               ...
               if( get_post_format($item->get_id()) ){
				...
				?>
				<li class="lesson-format-<?php echo esc_attr($lesson_format); ?> <?php echo esc_attr( implode( ' ', $item->get_class() ) ); ?>" data-id="<?php echo esc_attr( $item->get_id() ); ?>">
